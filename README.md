...
- [ ] **房主控制**：创建房间的玩家拥有开始游戏、修改设置、重置房间的权限。
- [ ] **房间参数配置**：可设置初始速度、起始行数、是否开启加时赛、是否开启垃圾行等。
- [ ] **房间列表 / 快速匹配（远期考虑）**：不依赖第三方后台，仅在当前本地服务器架构可行时再考虑。

### 第四阶段：应用化与体验

- [ ] **视觉与动效优化**：更精致的消除动画、连击提示、主题切换、移动端按钮布局优化。
- [ ] **音效与震动反馈**：优化当前 Web Audio 音效，手机端加入震动反馈。
- [ ] **键盘与手柄支持**：除键盘外，支持游戏手柄操作。
- [ ] **离线单人模式强化**：排行榜、历史最高分、本地挑战模式。
